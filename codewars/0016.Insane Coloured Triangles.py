...
def get_one(a, b):
    if a == b:
        ...
        return 'RGB'.translate(str.maketrans({a: None, b: None}))

def triangle(row):
    while len(row) != 1:
        new_row = ''.join([get_one(i, row[_index+1]) for _index, i in enumerate(row[:-1])])
        row = new_row
    return row



print(triangle('RBRGBRBGGRRRBGBBBGGBGBBBGGBBGGRGBRBGGRRRBGBBBGGBGBBBGGBBGGBRBGGRRRBGBBBGGBGBBBGGBBGGRGBRBGGRRRBGBBBGGBGBBBGGBBGG'))
